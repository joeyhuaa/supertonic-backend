class AudiosController < ApplicationController
  ...
  def new
    ...
    # todo - need to save song metadata on DB, and link to actual file on S3
    # ? - use Project model, has_many_attached :songs?
    # ? - use Song model, has_one_attached :song, then tie that to Project?

    @audio = Audio.new(audio_params)
    @audio.songs.attach(audio_params[:songs])
    # byebug
    if @audio.save
  ...
  private
    ...
    def audio_params
      params.require(:audio).permit(songs: [])
    end
end
